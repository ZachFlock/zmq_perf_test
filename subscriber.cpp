...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        ...
        cerr << "Example: " << argv[0] << " tcp://127.0.0.1:5555 10000 256\n";
        return 1;
    }
    ...
    try {
        ...
        for (long long i = 0; i < num_messages; ++i) {
            zmq::message_t msg;
            zmq::recv_result_t ok = sub.recv(msg, zmq::recv_flags::none);
            if (!ok.has_value()) {
                cerr << "Receive interrupted\n";
                break;
            }

            if (msg.size() < 8) {
                cerr << "Warning: received message smaller than 8 bytes (" << msg.size() << ")\n";
                continue;
            }
            if (msg.size() != expected_size) {
                // not fatal, but inform
                cerr << "Note: received message size " << msg.size()
                     << " differs from expected " << expected_size << '\n';
            }

            uint64_t ts_ns = read_be_u64(msg.data()); // timestamp embedded by publisher (big-endian)
        ...
        if (lat_ns.empty()) {
            cerr << "No latency samples collected\n";
            return 3;
        }
